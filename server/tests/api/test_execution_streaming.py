...
from tests.api.conftest import make_execution_record, make_mock_session


def _patch_execution_lookup(mock_user, slug: str):
    record = make_execution_record(id="exec-1", slug=slug, user_id=mock_user.id)
    return (
        patch("druids_server.api.routes.executions.get_session", make_mock_session()),
...
def test_streaming_sse_and_websocket_match_filtering_and_done(client, mock_user, mock_execution, execution_registry):
    ...
    registry = execution_registry[user_id]

    def fake_read_from(_execution_id: str, _line_num: int):
        if _line_num == 0:
...
def test_topology_event_streams_through_sse(client, mock_user, mock_execution, execution_registry):
