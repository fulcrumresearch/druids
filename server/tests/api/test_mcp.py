...
class TestRemoteExec:
    def test_remote_exec_forwards_user_and_timeout(self, client):
        """remote_exec should pass optional execution parameters through to the sandbox."""
        sandbox = MagicMock()
        sandbox.instance_id = "morph_123"
        sandbox.exec = AsyncMock(return_value=MagicMock(stdout="ok", stderr="", exit_code=0))

        from unittest.mock import patch

        with patch("druids_server.api.routes.mcp.resolve_sandbox", new_callable=AsyncMock, return_value=sandbox):
            response = client.post(
                "/remote-exec",
                json={
                    "execution_slug": SLUG,
        ...
        assert response.status_code == 200
        sandbox.exec.assert_awaited_once_with("iptables -L", user="root", timeout=30)
