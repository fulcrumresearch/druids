...
class BridgeRelayHub:
    ...
    async def register(self, bridge_id: str, token: str) -> None:
        async with self._lock:
            self._sessions[bridge_id] = RelaySession(bridge_id=bridge_id, token=token)
    ...
    async def read_output(self, bridge_id: str) -> bytes:
        ...
        if not session:
            raise ConnectionError(f"Unknown bridge_id: {bridge_id}")
        return await session.incoming.get()
...
class AgentConnection:
    ...
    _reader: RelayReader = field(init=False)
    _writer: RelayWriter = field(init=False)
    ...
    def __post_init__(self):
        ...
        self._reader = RelayReader(self.bridge_id)
        self._writer = RelayWriter(self.bridge_id)
    ...
    async def _handle_permission_request(self, params: dict) -> dict:
        ...
        logger.warning(f"No options to approve for tool: {tool_name}")
    ...
    async def _dispatch_method(self, method: str, params: Any | None, is_notification: bool) -> Any | None:
        ...
        logger.debug(f"Dispatch: {method} (notification={is_notification})")
    ...
    async def new_session(
        ...
        logger.info(f"Creating session with {len(acp_mcp_servers)} MCP servers: {[s.name for s in acp_mcp_servers]}")
        req = NewSessionRequest(cwd=cwd, mcpServers=acp_mcp_servers, field_meta=meta)
        req_dict = req.model_dump(by_alias=True)
        logger.info(f"session/new request: {list(req_dict.keys())}")
        result = await self.send_request("session/new", req_dict)
        logger.info(f"session/new result: {list(result.keys()) if isinstance(result, dict) else result}")
    ...
    async def set_model(self, model_id: str) -> None:
        ...
        logger.info(f"Set model to {model_id} for session {self.session_id}")
    ...
    async def prompt_nowait(self, text: str) -> None:
        ...
        request_id = self.connection._next_request_id
        self.connection._next_request_id += 1
        future = self.connection._state.register_outgoing(request_id, "session/prompt")
        payload = {"jsonrpc": "2.0", "id": request_id, "method": "session/prompt", "params": params}
        await self.connection._sender.send(payload)
        self.connection._notify_observers(StreamDirection.OUTGOING, payload)
