...
def upgrade() -> None:
    ...
    op.add_column("execution", sa.Column("program_id", sa.Uuid(), nullable=True))
    op.create_index(op.f("ix_execution_program_id"), "execution", ["program_id"])
    op.create_foreign_key("fk_execution_program_id", "execution", "program", ["program_id"], ["id"])


def downgrade() -> None:
    """Remove program table and execution.program_id."""
    op.drop_constraint("fk_execution_program_id", "execution", type_="foreignkey")
    op.drop_index(op.f("ix_execution_program_id"), table_name="execution")
    op.drop_column("execution", "program_id")
