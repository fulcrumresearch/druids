...
class Execution:
    ...
    def __post_init__(self) -> None:
        self._captioner = CaptionSummarizer(self.emit)
    ...
    async def agent(
        ...
        *,
        ...
        share_machine_with: str | Agent | None = None,
        mcp_servers: dict[str, Any] | None = None,
    ) -> Agent:
        """Create, provision, and connect an agent. Returns the live Agent."""
        share_name = share_machine_with.name if isinstance(share_machine_with, Agent) else share_machine_with
        ...
        try:
            ...
            t1 = time.monotonic()
            logger.info("Agent '%s': machine ready in %.2fs (instance=%s)", name, t1 - t0, machine.instance_id)
            ...
            logger.info(
                "Agent '%s': bridge ready, session deferred, total=%.2fs",
                name,
                t2 - t0,
            )
            ...
            if machine and not share_name and name not in self.agents:
                try:
                    await machine.stop()
                except Exception:
