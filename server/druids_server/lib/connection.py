...
class AgentConnection:
    ...
    @staticmethod
    def _build_mcp_servers(mcp_servers: list[dict] | None) -> list[HttpMcpServer]:
        """Convert dicts to ACP HttpMcpServer objects."""
        if not mcp_servers:
            return []
        result = []
    ...
    async def new_session(
        ...
        acp_mcp_servers = self._build_mcp_servers(mcp_servers)
    ...
    async def resume_session(
        ...
        """Resume an existing session by ID. Returns the session_id."""
        acp_mcp_servers = self._build_mcp_servers(mcp_servers)
        logger.info("Resuming session %s with %d MCP servers", session_id, len(acp_mcp_servers))
        req = ResumeSessionRequest(
            sessionId=session_id,
            cwd=cwd,
            mcpServers=acp_mcp_servers if acp_mcp_servers else None,
        )
        await self.send_request("session/resume", req.model_dump(by_alias=True))
