...
class Agent:
    ...
    # Set by Execution.fork_agent(context=True) to resume instead of creating a new session
    _resume_session_id: str | None = field(default=None, repr=False)
    ...
    async def _ensure_session(self) -> None:
        ...
        async with self._session_lock:
            ...
            if self._resume_session_id:
                self.session_id = await self._resume_acp_session(
                    self.config,
                    self._acp_config,
                    self._resume_session_id,
                    self.connection,
                )
                logger.info("Agent '%s': session resumed (deferred), session_id=%s", self.name, self.session_id)
            else:
    ...
    @staticmethod
    def _collect_mcp_servers(config: AgentConfig, acp: ACPConfig) -> list[dict]:
        """Build the MCP server list for session creation or resume."""
        base_url = str(settings.base_url)
        token = acp.env.get("DRUIDS_ACCESS_TOKEN", "")
        servers = [
            {
                "name": "druids",
                "url": f"{base_url}/amcp/",
        ...
        if acp.mcp_servers:
            for name, srv_config in acp.mcp_servers.items():
                servers.append({"name": name, **srv_config})
        return servers
    ...
    async def _create_acp_session(
        ...
        mcp_servers = cls._collect_mcp_servers(config, acp)
    ...
    async def _resume_acp_session(
        ...
        """Resume an ACP session with MCP servers. Returns the session ID."""
        mcp_servers = cls._collect_mcp_servers(config, acp)
        logger.info("Resuming session %s for '%s' with %d MCP server(s)", session_id, config.name, len(mcp_servers))
