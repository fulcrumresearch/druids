...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        async with mcp_app.lifespan(app), agent_mcp_lifespan():
            yield
    ...
    # Agent MCP: mount as a Starlette route so it takes priority over the
    # dashboard catch-all. Direct ASGI forwarding (no response buffering).
    app.routes.insert(0, Mount("/amcp", app=create_agent_mcp_app()))
    ...
    if dashboard_dist.is_dir():
        ...
        async def serve_dashboard(path: str):
            ...
            if path.startswith("api/") or path.startswith("mcp/") or path.startswith("amcp"):
