...
@router.post(
    "/executions/{slug}/agents/{source_name}/fork", tags=["executions"], dependencies=[Depends(require_driver)]
)
async def fork_agent(
    ...
    ex = _get_runtime_execution(executions, slug)
    source = _get_runtime_agent(ex, source_name)
...
async def runtime_ready(
    ...
    """Signal that the execution runtime is ready. Registers client event names and agent order."""
    ex = _get_runtime_execution(executions, slug)
    for event_name in request.client_events:
        ex._client_event_names.add(event_name)
    if request.agent_order:
        ex._agent_order = request.agent_order
        # Re-emit topology with corrected agent order so the frontend picks it up.
        if ex.edges:
            execution_trace.topology(ex.user_id, ex.slug, ex.all_agent_names(), ex.edges)
    return {"status": "ready"}
...
async def emit_event(
    ...
    ex = _get_runtime_execution(executions, slug)
    await ex.emit(request.event, request.data)
...
@router.get(
    "/executions/{execution_slug}/agents/{agent_name}/trace",
