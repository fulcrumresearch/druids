...
from druids_server.db.models.program import get_or_create_program
...
class CreateExecutionRequest(BaseModel):
    ...
    files: dict[str, str] | None = None  # path -> content, written to sandbox before program runs
...
async def create_execution_endpoint(
    ...
    # Resolve devbox, or use the default base image if none specified
    devbox = None
    if request.devbox_name or request.repo_full_name:
        async with get_session() as db:
            ...
            if not devbox:
                ...
                raise HTTPException(404, f"No devbox for '{label}'. Run 'druids devbox create' first.")
    ...
    if devbox:
        ...
        # No devbox: use the default base image
        devbox_machine = Machine(snapshot_id="")

    program_args = request.args or {}
    repo_full_name = (devbox.repo_full_name if devbox else None) or None

    # Save program and create execution record
    async with get_session() as db:
        program = await get_or_create_program(db, caller.user.id, request.program_source)
    ...
    ex = Execution(
        ...
        devbox_id=devbox.id if devbox else None,
...
@router.get("/executions/{execution_slug}", tags=["executions", "mcp-driver"], operation_id="get_execution")
async def get_execution_endpoint(
    ...
    return {
        ...
        "program_id": str(record.program_id) if record.program_id else None,
        "started_at": record.started_at.isoformat() if record.started_at else None,
        "stopped_at": record.stopped_at.isoformat() if record.stopped_at else None,
        "agents": list(runtime.all_agent_names()) if runtime else record.agents,
        ...
        "edges": runtime.edges if runtime else record.edges,
    }
...
@router.patch("/executions/{execution_slug}", tags=["executions", "mcp-driver"], operation_id="update_execution")
...
@router.get("/executions/{execution_slug}/diff", tags=["executions", "mcp-driver"], operation_id="get_execution_diff")
...
@router.get("/executions/{execution_slug}/activity", tags=["executions", "mcp-driver"], operation_id="get_execution_activity")
...
@router.get("/executions/{execution_slug}/ssh", tags=["executions"], operation_id="get_execution_ssh")
...
@router.get("/executions/{execution_slug}/stream", tags=["executions"], operation_id="stream_execution")
...
@router.post(
    "/executions/{execution_slug}/agents/{agent_name}/message",
    tags=["executions"],
...
@router.websocket("/executions/{execution_slug}/ws")
