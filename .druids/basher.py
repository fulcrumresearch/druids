...
async def program(ctx, repo_full_name="", task_name="", task_spec="", slack_token=""):
    ...
    async def spawn_task(task_name: str, task_spec: str):
        ...
        await ctx.emit("task_spawned", {"name": task_name, "implementor": impl_name, "reviewer": review_name})
        ...
        async def on_approve(pr_url: str = "", summary: str = ""):
            ...
            completed_tasks.append({"name": task_name, "pr_url": pr_url})
            await ctx.emit("task_completed", {"name": task_name, "pr_url": pr_url})
            if finder:
                await finder.send(f"Task '{task_name}' completed. PR: {pr_url}\nActive tasks: {len(active_tasks)}.")
            else:
                await ctx.done(f"PR created: {pr_url}")
        ...
        async def on_feedback(feedback: str = ""):
            ...
            if rejections >= MAX_REVIEW_ROUNDS:
                active_tasks.pop(impl_name, None)
                abandoned_tasks.append({"name": task_name, "rounds": rejections, "last_feedback": feedback})
                await ctx.emit("task_abandoned", {"name": task_name, "rounds": rejections})
    ...
    async def handle_input(text=""):
        """Route driver input to the finder."""
        await finder.send(f"[Driver]: {text}")
        return {"ack": True}
