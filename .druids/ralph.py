...
async def program(ctx, spec="", task_name="", repo_full_name="", promise="", max_iterations=""):
    ...
    def get_state():
        ...
        return {
            ...
            "promise": promise,
            "task_name": task_name or "Ralph task",
            "agents": agents,
        }
    ...
    async def on_done(summary: str = ""):
        ...
        if promise in summary:
            ...
            await ctx.done(f"Ralph complete in {iteration} iteration(s). {summary}")
            return "Done."

        if iteration >= max_iter:
            await ctx.done(f"Ralph stopped after {iteration} iterations (limit: {max_iter}). Last summary: {summary}")
            return "Max iterations reached."

        # No promise found. Re-feed the prompt.
        await ctx.emit("iteration", {"iteration": iteration, "max_iterations": max_iter})
        await worker.send(
