...
`ctx.done(result)` signals successful completion. `ctx.fail(reason)` signals failure. Without one of these, the execution runs until its TTL expires. The TTL is set when creating an execution (in seconds, default 0 which defers to the server maximum of 2 hours). When the TTL fires, all agents are stopped and the execution is marked as failed.

Most programs end via an event handler calling `ctx.done()` or `ctx.fail()`. Long-lived programs that accept client events indefinitely call `await ctx.wait()` at the end of the program function to block until one of these signals fires.
...
Programs can also push events to connected clients with `ctx.emit()`:

```python
@builder.on("surface")
async def on_surface(title: str = "", body: str = ""):
    """Surface a decision or consideration for the driver."""
    ctx.emit("consideration", {"title": title, "body": body})
