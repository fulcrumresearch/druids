...
`ctx.done()` ends the execution. After this call, the server stops all agents and cleans up the sandboxes. The string passed to `done()` is recorded as the execution result and shown in the dashboard.
