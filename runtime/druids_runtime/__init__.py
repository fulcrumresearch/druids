...
class RuntimeAgent:
    ...
    async def exec(self, command: str, *, user: str = "agent", timeout: int | None = None) -> Any:
        ...
        await self._await_ready()
        resp = await self._ctx._remote_exec(self.name, command, user=user, timeout=timeout)
...
@dataclass
class RuntimeState:
    """Mutable key/value state associated with the running execution."""
    ...
    async def set(self, name: str, value: Any) -> None:
        """Store a state value and forward it to the server."""
        self._values[name] = value
        await self._ctx._emit_state(name, value)
...
class RuntimeContext:
    ...
    _state_values: dict[str, Any] = field(default_factory=dict, repr=False)
    _state: RuntimeState = field(init=False, repr=False)
    _server_started: bool = field(default=False, repr=False)
    _server_task: asyncio.Task | None = field(default=None, repr=False)

    def __post_init__(self) -> None:
        self._state = RuntimeState(self, self._state_values)

    @property
    def state(self) -> RuntimeState:
        """Expose runtime state for program-level variables."""
        return self._state
    ...
    def connect(self, a: RuntimeAgent, b: RuntimeAgent, *, direction: str = "both") -> None:
        ...
        # If the server is already running (wait() was called), push the
        # full topology immediately. Without this, agents created after
        # wait() (e.g. forked workers) are missing from the edge topology
        # and can't use send_file/download_file.
        if self._server_started:
            all_edges = [{"from": f, "to": t} for f, t in self._topology]
            asyncio.create_task(self._post("/edges", {"edges": all_edges}))
    ...
    async def _remote_exec(
        ...
        *,
        ...
        body: dict[str, Any] = {"execution_slug": self.slug, "agent_name": agent_name, "command": command}
        if user is not None:
            body["user"] = user
        ...
        if timeout is not None and timeout > 0:
            client_timeout = max(client_timeout, timeout + 30)
    ...
    async def _emit_state(self, name: str, value: Any) -> None:
        """Emit a state update as a standard client event."""
        try:
            await self._post("/emit", {"event": "program_state", "data": {"name": name, "value": value}})
        except Exception:
            logger.debug("Failed to emit state update %s", name, exc_info=True)
