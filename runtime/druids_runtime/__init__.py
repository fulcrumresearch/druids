...
class RuntimeContext:
    ...
    def emit(self, event: str, data: dict[str, Any] | None = None) -> None:
        ...
        asyncio.get_event_loop().create_task(self._post("/emit", {"event": event, "data": data}))

    def done(self, result: Any = None) -> None:
        """Signal successful completion."""
        asyncio.get_event_loop().create_task(self._request("PATCH", "", {"status": "completed", "result": result}))

    def fail(self, reason: str) -> None:
        """Signal failure."""
        asyncio.get_event_loop().create_task(self._request("PATCH", "", {"status": "failed", "reason": reason}))
