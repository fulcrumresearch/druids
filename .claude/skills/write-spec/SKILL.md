...
The agent runs on a VM that the user set up via `druids setup`. The repo is already cloned, dependencies are already installed, and the environment is ready to go. The agent does not need to clone, install, or configure anything. It can start coding immediately.
