...
def devbox_create(
    ...
    vcpus: int | None = typer.Option(None, "--vcpus", help="Number of vCPUs (default: 2)"),
    memory: int | None = typer.Option(None, "--memory", help="Memory in MB (default: 4096)"),
    disk: int | None = typer.Option(None, "--disk", help="Disk size in MB (default: 10240)"),
):
...
def devbox_list():
    ...
    for d in devboxes:
        ...
        resources = ""
        if d.get("vcpus") or d.get("memory_mb") or d.get("disk_mb"):
            parts = []
            if d.get("vcpus"):
                parts.append(f"{d['vcpus']}cpu")
            if d.get("memory_mb"):
                parts.append(f"{d['memory_mb']}MB")
            if d.get("disk_mb"):
                parts.append(f"{d['disk_mb']}MB disk")
            resources = f" [dim]({', '.join(parts)})[/dim]"
        console.print(f"[bold]{d['name']}[/bold]{repo}{resources} [{ready}]")
...
def exec(
    ...
    repo: str | None = typer.Option(None, "--repo", "-r", help="GitHub repo (owner/repo) to clone onto the base image"),
    ...
    devbox_name = devbox
    repo_full_name = repo
    if not no_setup and not devbox_name and not repo_full_name:
        repo_full_name = get_repo_from_cwd()
        if not repo_full_name:
            print_error("Provide --devbox, --repo, run from inside a git repo, or use --no-setup.")
            raise typer.Exit(1)
...
def _write_ssh_key(execution_slug: str, agent: str | None, private_key: str) -> Path:
    """Write an SSH private key to a stable path under ~/.druids/keys/."""
    keys_dir = Path.home() / ".druids" / "keys"
    keys_dir.mkdir(parents=True, exist_ok=True)
    suffix = f"-{agent}" if agent else ""
    key_path = keys_dir / f"{execution_slug}{suffix}.pem"
...
def execution_ssh(
    ...
    info: bool = typer.Option(False, "--info", help="Print SSH/SCP connection details instead of connecting"),
):
    """Open a shell on an execution's VM.

    With --info, prints the SSH key path and ready-to-use ssh/scp commands
    ...
    key_path = _write_ssh_key(execution_slug, agent, private_key)
    ssh_opts = "-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"
    port_flag = f"-p {port} " if port != 22 else ""
    port_flag_scp = f"-P {port} " if port != 22 else ""
    ...
    if info:
        ...
        if password:
            console.print(f"[bold]Pass:[/bold] {password}")
        console.print()
        console.print(f"[bold]SSH:[/bold]  ssh -i {key_path} {ssh_opts} {port_flag}{username}@{host}")
        console.print(
            f"[bold]SCP:[/bold]  scp -i {key_path} {ssh_opts} {port_flag_scp}local.txt {username}@{host}:/home/agent/"
        )
        return

    ssh_cmd_opts = ["-i", str(key_path), "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null"]
    if port != 22:
        ssh_cmd_opts += ["-p", str(port)]
    ...
    console.print(f"[dim]Key saved to {key_path}[/dim]")
    remote_cmd = "sudo -iu agent"

    subprocess.run(["ssh"] + ssh_cmd_opts + ["-t", f"{username}@{host}", remote_cmd])
...
def execution_connect(
    ...
    key_path = _write_ssh_key(execution_slug, agent, private_key)
    ssh_opts = ["-i", str(key_path), "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null"]
