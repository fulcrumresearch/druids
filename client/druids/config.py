...
def _get_server_dotenv() -> Path | None:
    """Find ``server/.env`` in the current git repo."""
    try:
        ...
        dotenv = Path(result.stdout.strip()) / "server" / ".env"
        return dotenv if dotenv.exists() else None
