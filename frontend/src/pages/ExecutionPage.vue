...
import ProgramStatePanel from '../components/ProgramStatePanel.vue'
...
// --- Lifecycle ---
const programState = computed(() => stream.programState.value)
const hasProgramState = computed(() => Object.keys(programState.value).length > 0)
...
async function initLive() {
  try {
    ...
    if (data.program_state) {
      stream.programState.value = { ...data.program_state }
    }
    stream.connect()
  } catch (e) {
...
<template>
  <div class="graph-page">
    ...
    <div ref="graphArea" class="graph-area">
      ...
      <!-- Program state (live mode, shown when state exists) -->
      <div v-if="!isDemo && hasProgramState" class="program-state-overlay">
        <ProgramStatePanel :state="programState" />
      </div>
